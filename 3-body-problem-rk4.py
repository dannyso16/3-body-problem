...
STEPS = 200_000  # シミュレーションのステップ数
SKIP_FRAMES = 100     # 表示をさらに早送りするためのスキップフレーム数を短縮
...
class App:
    ...
    def precompute_simulation(self):
        ...
        for step in range(STEPS):
            ...
            # 実行状況を端末に出力
            if step % (STEPS // 100) == 0:  # 進捗を1%ごとに表示
                print(f"Precomputing simulation: {step / STEPS * 100:.1f}% completed")
    ...
    def draw(self):
        ...
        for i, body in enumerate(self.bodies):
            ...
            pyxel.circ(int(frame_data[i][0]), int(frame_data[i][1]), 2, body.color)
